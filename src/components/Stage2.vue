  ...
  <div class="detail">
    <div v-if="stageShow">
      ...
      <div class="showImage">
        ...
        <img v-if="this.complete" id="end" :src="this.finishedCompleteUrl" style="width: 25vw; height: 25vw">
        <el-progress v-if="!this.complete" type="circle" :percentage="this.progress"></el-progress>
      </div>
    </div>
  </div>
</template>
...
export default {
  name: 'Stage2',
  props: ['allData', 'stageShowTwo'],
  mounted () {
  },
  data () {
    return {
      stageShow: this.stageShowTwo,
    ...
    allData: {
      handler (nv, ov) {
        ...
        if (len >= 2) {
          ...
          let url = this.stageData[1]['url']
          this.startCompleteUrl = this.baseUrl + '/' + url + '.png'
          if (this.stageData[1]['progress'] === 100) {
            this.complete = true
            // 去掉-g，相当于去掉最后两个字符
