    ...
    <div v-if="stageShow">
      <div class="container" style="width: 50%;margin-left: 0">
        <div class="row align-items-center">
          ...
          <div class="col-10">
            <b-card-text class="text-left" style="color:#F56C6C">对图像进行进一步的处理</b-card-text>
          </div>
        </div>
    ...
    allData: {
      handler (nv, ov) {
        ...
        if (len >= 3) {
          ...
          let url = this.stageData[2]['url']
          this.startCompleteUrl = this.baseUrl + '/' + url + '.png'
          if (this.stageData[2]['progress'] === 100) {
            this.complete = true
            // 去掉-g，相当于去掉最后两个字符
